    ...
    public class BookRepository : IBookRepository
    {
        ...
        public async Task<List<Book>> GetAllBookAsync()
        {
            List<Book> book = await _context.Books
                .Include(b => b.Author).Include(b => b.Category).Include(b => b.Type).ToListAsync();
            return book;
        }
        ...
        public async Task<Book> GetByIdBookAsync(int? id)
        {
            ...
            Book book = await _context.Books
              .Include(b => b.Author).Include(b => b.Category).Include(b => b.Type).FirstOrDefaultAsync(b => b.Id == id);
            if (book == null)
            {
        ...
        public async Task AddAsync(Book book, int typeId, int categoryId, int authorId)
        {
        ...
        public async Task<Book> UpdateAsync(int id, Book book, int typeId, int categoryId, int authorId)
        {
            if (book == null)
            {
                throw new ArgumentException(nameof(book), "the book object cannot be null");
        ...
        public async Task<(IEnumerable<Book>, int)> GetBooksWithFiltersAsync(BookParams bookParams)
        {
            IQueryable<Book> query = _context.Books
                .Include(b => b.Type).Include(b => b.Category).Include(b => b.Author).AsQueryable();
            //Search
