...
namespace API.Controllers
{
    public class BookController : BaseApiController
    {
        ...
        [HttpPost("create")]
        public async Task<ActionResult<Book>> CreateBook([FromForm] Book book, [FromForm] int typeId, [FromForm] int categoryId, [FromForm] int authorId)
        {
            try
            {
                await _bookRepo.AddAsync(book, typeId, categoryId, authorId );
        ...
        //[Authorize(Roles = "Admin")]
        [HttpPut("update/{id}")]
        public async Task<ActionResult<Book>> UpdateBook(int id, [FromForm] Book book, [FromForm] int typeId, [FromForm] int categoryId, [FromForm] int authorId) 
        {
            return await _bookRepo.UpdateAsync(id, book, typeId, categoryId, authorId);
        }
        [Cached(600)]
        //[Authorize(Roles = "Admin,Client")]
        [HttpGet("download/{id}")]
